	...
	return BaseController.extend("LoginPageLoginPage.controller.ListOfMachines", {
		...
		_onButtonPress: function(oEvent) {
			...
			var confirmHandler = function(oConfirmEvent) {
				...
				vValueLT = oDialog.getModel().getProperty("/DueOrders/vValueLT");
				vValueGT = oDialog.getModel().getProperty("/DueOrders/vValueGT");
				if (vValueLT !== "" || vValueGT !== "") {
					oFilter = this.getCustomFilter("DueOrders", vValueLT, vValueGT);
					oBindingData.filters.push(oFilter);
					sFilterString = sFilterString ? sFilterString + ', ' : 'Filtered by: ';
					sFilterString += this.getCustomFilterString(true, "DueOrders", oFilter.sOperator, vValueLT, vValueGT);
				}
			...
			function resetFiltersHandler() {
				...
				oDialog.getModel().setProperty("/DueOrders/vValueLT", "");
				oDialog.getModel().setProperty("/DueOrders/vValueGT", "");
			...
			function updateDialogData(filters) {
				var mParams = {
					...
					success: function(oData) {
						...
						oJsonModelDialogData["DueOrders"] = {
							vValueLT: (oDialogModel && oDialogModel.getProperty("/DueOrders")) ? oDialogModel.getProperty("/DueOrders/vValueLT") : "",
							vValueGT: (oDialogModel && oDialogModel.getProperty("/DueOrders")) ? oDialogModel.getProperty("/DueOrders/vValueGT") : ""
						};
		...
		_onButtonPress1: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				...
				this.doNavigate("ListOfOrders", oBindingContext, fnResolve, "");
			}.bind(this)).catch(function(err) {
				if (err !== undefined) {
					MessageBox.error(err.message);
				}
